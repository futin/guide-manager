...
- **Default off, persisted per-origin** under `guide-manager:bionic`. Bionic
  ...
  offered, not imposed.
- **The key is shared, and so is the control.** guide-manager's own Settings
  page writes that same key, and since v2 the aid listens for the `storage`
  event it raises — so a guide framed in the app repaints the moment the
  setting changes there, without a reload. The in-guide panel keeps working as
  a local override: both routes write one key, and whichever moved last is what
  both surfaces show.
