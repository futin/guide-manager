 ...
 * Allowed ranges.
 *
 * ⚠️ These are NOT chosen here. They are copied from `readState()` in
 * `assets/bionic.js`, which is vendored and must not be edited. If the two ever
 * diverge, the aid silently discards whatever Settings stored and falls back to
 * its own defaults — the switch would move and nothing would change.
 */
export const LIMITS = {
